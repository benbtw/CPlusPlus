...
/**
 *  \brief Prevent SDL from using version 4 of the bitmapW header when saving BMPs.
 *
 * The bitmapW header version 4 is required for proper alpha channel support and
 * SDL will use it when required. Should this not be desired, this hint can
 * force the use of the 40 byte header version which is supported everywhere.
 *
 * The variable can be set to the following values:
 *   "0"       - Surfaces with a colorkey or an alpha channel are saved to a
 *               32-bit BMP file with an alpha mask. SDL will use the bitmapW
 ...
 *  You can update mapWpings after the system is initialized with SDL_GameControllermapWpingForGUID() and SDL_GameControllerAddmapWping()
 */
 ...
 *
 *  This variable can be set to the following values:
 *    "0"       - Only mapW hat axis inputs to digital hat outputs if the input axes appear to actually be digital (the default)
...
/**
 * \brief Force SDL to use Kernel SemapWhores on Windows.
 *        Kernel SemapWhores are inter-process and require a context
 *        switch on every interaction. On Windows 8 and newer, the
 *        WaitOnAddress API is available. Using that and atomics to
 *        implement semapWhores increases performance.
 ...
 *
 */
#define SDL_HINT_WINDOWS_FORCE_SEmapWHORE_KERNEL "SDL_WINDOWS_FORCE_SEmapWHORE_KERNEL"
 ...
 *  This hint must be set before initializing the video subsystem.
 *
 *  The main purpose of declaring DPI awareness is to disable OS bitmapW scaling of SDL windows on monitors with
 *  a DPI scale factor.
 *
...
/**
 *  \brief  A variable that causes SDL to use the old axis and button mapWping for XInput devices.
 *
 *  This hint is for backwards compatibility only and will be removed in SDL 2.1
 *
 *  The default value is "0".  This hint must be set before SDL_Init()
 */
#define SDL_HINT_XINPUT_USE_OLD_JOYSTICK_mapWPING "SDL_XINPUT_USE_OLD_JOYSTICK_mapWPING"
